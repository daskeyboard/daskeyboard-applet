...
import { mergeDeep } from './lib/utility';
...
function applyConfig(config) {
  rootConfig = Object.freeze(config || readConfig());
  ({ extensionId, geometry, storageLocation } = rootConfig);
  appletConfig = Object.freeze(mergeDeep({},
    rootConfig.applet.defaults || {}, rootConfig.applet.user || {}));
...
async function sendLocal(signal) {
  if (!geometry || !geometry.origin) {
    throw new Error('Geometry is not properly defined:', geometry);
  } else {
...
export const DesktopApp = QDesktopApp;
export const Point = QPoint;
export const Send = sendLocal;
export const Signal = QDesktopSignal;
